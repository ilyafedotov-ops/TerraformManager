...
<section class="space-y-8">
    ...
    {:else if activeStep === 'destination'}
        <DestinationStep
            ...
            activeProjectId={activeProjectValue?.id ?? null}
            autoSaveEnabled={autoSaveEnabled}
            assetName={destinationAssetName}
            assetDescription={destinationAssetDescription}
            assetTags={destinationAssetTags}
            hasContext={Boolean(generatorContexts[activeGenerator])}
            error={destinationError}
            saving={destinationSaving}
            on:projectChange={(event) => updateActiveProject((event.detail.id as string) ?? '')}
            on:toggleAutoSave={(event) => (autoSaveEnabled = Boolean(event.detail.enabled))}
            on:assetNameChange={(event) => (destinationAssetName = event.detail.value as string)}
            on:assetDescriptionChange={(event) => (destinationAssetDescription = event.detail.value as string)}
            on:assetTagsChange={(event) => (destinationAssetTags = event.detail.value as string)}
            on:save={handleSaveDestination}
            on:goToReview={moveToReview}
            on:goToConfigure={moveToConfigure}
        />
    {:else}
