...
const {
	...
	bannerContext = 'Review runs are logged to your active workspace alongside generator history and artifacts.',
	heading = 'Upload Terraform for analysis'
}: Props = $props();
...
const submitScan = async (event?: Event, payload?: ScanFormData) => {
	...
	if (!projectId && !projectSlug) {
		error = 'Project context missing. Select a workspace before running a scan.';
		return;
	}
...
<section class="space-y-8">
	<header class="space-y-3">
		...
		<p class="max-w-3xl text-sm text-slate-500">
			Drop one or more Terraform modules (individual
			<code class="rounded bg-slate-50 px-1 py-0.5 text-xs text-slate-600">.tf</code> files or zipped directories). The
			backend will unpack archives, apply the configured review rules, and optionally persist the report for later lookup.
		</p>
	</header>

	{#if showWorkspaceBanner}
		<ProjectWorkspaceBanner context={bannerContext} />
	{/if}
	...
	{#if result}
		<ScanSummary
			reportId={(result.id ?? null) as string | null}
