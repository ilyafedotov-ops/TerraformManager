...
<div class="space-y-2">
    <h2 class="text-3xl font-semibold text-slate-700">Welcome back 👋</h2>
    ...
    <label class="block space-y-2 text-sm font-medium text-slate-600">
        ...
        <p class="text-xs text-slate-400">
            Hint: if <code class="rounded bg-slate-200 px-1 py-0.5 text-xs text-slate-600">TFM_API_TOKEN</code> is still configured, paste it here
	...
	<button
		class="inline-flex w-full items-center justify-center rounded-2xl bg-gradient-to-r from-sky-500 via-indigo-500 to-blue-600 px-6 py-3 text-base font-semibold text-white shadow-lg shadow-sky-300/50 transition hover:from-sky-400 hover:via-indigo-400 hover:to-blue-500 focus:outline-none focus:ring-2 focus:ring-sky-200"
		type="submit"
	>
		Sign in
	</button>
	...
	<div class="flex flex-col gap-2 text-xs text-slate-400 sm:flex-row sm:items-center sm:justify-between">
		<a class="font-semibold text-sky-600 hover:text-sky-700" href="/forgot-password">Forgot token?</a>
		<div>
			New here?
			<a class="font-semibold text-sky-600 hover:text-sky-700" href="/register">Create workspace access</a>
		</div>
	</div>
</form>
