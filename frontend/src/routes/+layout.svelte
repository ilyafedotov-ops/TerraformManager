	...
	{#if toasts.length}
		<div class="pointer-events-none fixed bottom-6 right-6 z-50 flex max-w-sm flex-col gap-4">
			{#each toasts as toast (toast.id)}
				<div
					class={`pointer-events-auto flex items-start gap-3 rounded-2xl border px-4 py-3 shadow-lg shadow-slate-300/30 transition ${variantClass(toast.variant)}`}
				>
					<div class="flex-1 text-sm">{toast.message}</div>
					<button
						type="button"
						class="rounded-full border border-slate-200 px-2 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500 hover:border-slate-300 hover:text-slate-700"
