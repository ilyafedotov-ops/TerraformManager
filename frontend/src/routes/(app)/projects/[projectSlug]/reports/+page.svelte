		...
		<div class="space-y-6">
			...
			{#if !isLoading && reports.length === 0}
				<div class="rounded-3xl border border-slate-200 bg-slate-50 px-6 py-6 text-sm text-slate-500">
					{#if projectSlug || projectIdentifier}
						No saved reports are linked to this project yet. Run
						<code class="rounded bg-slate-100 px-1 py-0.5 text-xs text-slate-600">
							python -m backend.cli scan sample {projectSlug ? `--project-slug ${projectSlug}` : `--project-id ${projectIdentifier}`}
							--out tmp/report.json
						</code>
						or upload Terraform from the Review tab to log a report.
					{:else}
