...
RULES_REGISTRY: Dict[str, RuleMetadata] = {
    ...
    "AZ-SERVICEBUS-IDENTITY": RuleMetadata(
        rule_id="AZ-SERVICEBUS-IDENTITY",
        title="Service Bus namespace '{resource}' is missing a managed identity",
        severity="HIGH",
        description="Service Bus namespaces should use managed identities to authenticate with dependent resources (Key Vault, Storage) and avoid shared keys.",
        recommendation="Add an `identity` block with `type = \"SystemAssigned\"` (or user-assigned IDs) on namespace '{resource}'.",
        knowledge_ref="knowledge/azure_servicebus_best_practices.md#managed-identities",
    ),
    "AZ-SERVICEBUS-DIAGNOSTICS": RuleMetadata(
        rule_id="AZ-SERVICEBUS-DIAGNOSTICS",
        title="Service Bus namespace '{resource}' is missing diagnostic settings",
        severity="MEDIUM",
        description="Without diagnostic settings, Service Bus operations and audit logs are not captured for investigation or monitoring.",
        recommendation="Create `azurerm_monitor_diagnostic_setting` for namespace '{resource}' streaming OperationalLogs and metrics to Log Analytics.",
        knowledge_ref="knowledge/azure_servicebus_best_practices.md#diagnostics",
    ),
    "AZ-SERVICEBUS-PRIVATE-ENDPOINT": RuleMetadata(
        rule_id="AZ-SERVICEBUS-PRIVATE-ENDPOINT",
        title="Service Bus namespace '{resource}' does not use a private endpoint",
        severity="HIGH",
        description="Enabling private endpoints restricts Service Bus access to trusted virtual networks, reducing exposure of shared access keys and management endpoints.",
        recommendation="Provision `azurerm_private_endpoint` targeting namespace '{resource}' with `subresource_names = [\"namespace\"]`.",
        knowledge_ref="knowledge/azure_servicebus_best_practices.md#networking",
    ),
    "AZ-FUNCTION-HTTPS": RuleMetadata(
        rule_id="AZ-FUNCTION-HTTPS",
        title="Function App '{resource}' is missing HTTPS-only enforcement",
        severity="HIGH",
        description="Function Apps should require TLS and enforce minimum TLS 1.2 to protect HTTP triggers and SCM endpoints.",
        recommendation="Set `https_only = true` and configure `site_config` with `minimum_tls_version = \"1.2\"` for function app '{resource}'.",
        knowledge_ref="knowledge/generated/azure_function-app.md#azure-function-app-baseline",
    ),
    "AZ-FUNCTION-FTPS-DISABLED": RuleMetadata(
        rule_id="AZ-FUNCTION-FTPS-DISABLED",
        title="Function App '{resource}' allows legacy FTPS access",
        severity="MEDIUM",
        description="FTPS introduces credential theft risk; disable it unless explicitly required and brokered through secure channels.",
        recommendation="Within `site_config`, set `ftps_state = \"Disabled\"` (or restrict to `FtpsOnly`) for function app '{resource}'.",
        knowledge_ref="knowledge/generated/azure_function-app.md#azure-function-app-baseline",
    ),
    "AZ-FUNCTION-DIAGNOSTICS": RuleMetadata(
        rule_id="AZ-FUNCTION-DIAGNOSTICS",
        title="Function App '{resource}' is missing diagnostic settings",
        severity="MEDIUM",
        description="Streaming platform logs to Log Analytics enables alerting on failures and security events for Function Apps.",
        recommendation="Create `azurerm_monitor_diagnostic_setting` for function app '{resource}' forwarding logs and metrics to Log Analytics.",
        knowledge_ref="knowledge/generated/azure_function-app.md#azure-function-app-baseline",
    ),
    "AZ-APIM-TLS12": RuleMetadata(
        rule_id="AZ-APIM-TLS12",
        title="API Management '{resource}' permits TLS 1.0/1.1",
        severity="HIGH",
        description="Allowing legacy TLS versions weakens API gateway security posture and may violate compliance standards.",
        recommendation="In `custom_properties`, disable TLS 1.0 and 1.1 by setting the corresponding gateway protocol flags to `false` for '{resource}'.",
        knowledge_ref="knowledge/generated/azure_api-management.md#azure-api-management-premium-baseline",
    ),
    "AZ-APIM-PRIVATE-NETWORK": RuleMetadata(
        rule_id="AZ-APIM-PRIVATE-NETWORK",
        title="API Management '{resource}' lacks private network integration",
        severity="HIGH",
        description="Private networking protects API gateways from public exposure and enables downstream firewalling.",
        recommendation="Set `virtual_network_type` to `Internal` or `External` and provide a `virtual_network_configuration` with a private subnet for '{resource}'.",
        knowledge_ref="knowledge/generated/azure_api-management.md#azure-api-management-premium-baseline",
    ),
    # Kubernetes
